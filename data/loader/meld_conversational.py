...
class MELDConversationalLoader(Loader):
    ...
    def post_load(self, data_list):
        ...
        data_list.sort(key=lambda x: int(x[data.constants.MELD_DIALOG_ID]))

        for _, dialog_data in itertools.groupby(data_list, lambda x: x[data.constants.MELD_DIALOG_ID]):
            dialog_data = list(dialog_data)
            
            dialog_speakers = {x: i for i, x in enumerate(set([x[data.constants.MELD_SPEAKER] for x in dialog_data]))}
            ...
            for item in dialog_data:
                ...
                item[data.constants.DATA_SPEAKER_MASK] = data.utils.one_hot_encode(dialog_speakers[item[data.constants.MELD_SPEAKER]], 9)
                item[data.constants.DATA_UTTERANCE_MASK] = [1]

            dialog_data.sort(key=lambda x: int(x[data.constants.MELD_UTTERANCE_ID]))
